...
Object3dDetector::Object3dDetector() {
  ...
  private_nh.param<std::string>("frame_id", frame_id_, "iris_interface_node");
  //court -> -1~10, far seat -> 0~5
  private_nh.param<float>("z_limit_min", z_limit_min_, -10);
  private_nh.param<float>("z_limit_max", z_limit_max_, 10);

  //far seat -> 3~10
  private_nh.param<int>("cluster_size_min", cluster_size_min_, 6);
  private_nh.param<int>("cluster_size_max", cluster_size_max_, 300);
  private_nh.param<float>("human_probability", human_probability_, 0.3);
  ...
  pcl::io::loadPCDFile<pcl::PointXYZI> ("/home/aemass/aemass_ws/data/7615.100049765.pcd", *background_cloud);
...
void Object3dDetector::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2) {
  ...
  extractChangeParts(pcl_pc, pcl_pc_change, resolution);
...
const int nested_regions_ = 77;
int zone_[nested_regions_] = {2,3,3,3,3,3,3,2,3,3,3,3,3,3,2,3,3,3,3,3,3,2,3,3,3,3,3,3
,2,3,3,3,3,3,3,2,3,3,3,3,3,3,2,3,3,3,3,3,3,2,3,3,3,3,3,3,2,3,3,3,3,3,3,2,3,3,3,3,3,3,2,3,3,3,3,3,3}; // for more details, see our IROS'17 paper.
  ...
  for(int i = 0; i < nested_regions_; i++) {
    ...
    if(indices_array[i].size() > cluster_size_min_) {
      ...
      for(std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); it++) {
	   ...
	   (max[0]-min[0] < 0.2 || max[0]-min[0] > 0.8 ||
	    max[1]-min[1] < 0.2 || max[1]-min[1] > 1.2 ||
	    max[2]-min[2] < 0.3 || max[2]-min[2] > 2.0)) 
	  continue;
