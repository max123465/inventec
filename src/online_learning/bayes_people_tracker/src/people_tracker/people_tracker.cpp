...
void PeopleTracker::trackingThread() {
  ...
  //int ids_i = std::stoi(it->first);
  //std::string ids_s = std::to_string(((int)it->first) %200);
  ...
  cal_poses.push_back(tmp);
	...
	if(pub_detect.getNumSubscribers() || pub_pose.getNumSubscribers() || pub_pose_array.getNumSubscribers() || pub_people.getNumSubscribers()) {
      ...
      if(pub_detect.getNumSubscribers() || pub_pose.getNumSubscribers() || pub_pose_array.getNumSubscribers() || pub_people.getNumSubscribers())
	publishDetections(time_sec, closest_person_point, poses, vels, uuids, distances, angles, min_dist, angle, cal_poses, pids);
...
void PeopleTracker::publishDetections(double time_sec,
  ...
  result.calibrated_distances = cal_poses;
  result.pids = pids;
